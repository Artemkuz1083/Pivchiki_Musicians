...
export function FilterSheet({ isOpen, onClose, filters, setFilters }: Props) {
		...
		<div className='fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm transition-all'>
			<div className='w-full max-w-md bg-white rounded-t-[2.5rem] p-6 shadow-2xl animate-in slide-in-from-bottom duration-300'>
				...
				<div className='flex justify-between items-center mb-8'>
					<h2 className='text-2xl font-black text-[#1F1F1F] uppercase italic tracking-tighter'>
						Настройки поиска
					</h2>
					<button
						onClick={onClose}
				...
				<div className='space-y-8 overflow-y-auto max-h-[65vh] pr-2 custom-scrollbar'>
					...
					<section>
						...
						<div className='flex bg-gray-100 rounded-2xl p-1.5 gap-1'>
							{SKILL_LEVELS.map(level => (
								<button
									key={level.value}
									onClick={() =>
										setFilters({ ...filters, minSkill: level.value })
									}
						...
						<p className='mt-2 text-[10px] text-gray-400 italic text-center'>
							Будем искать музыкантов уровня "
							{SKILL_LEVELS[filters.minSkill].label}" и выше
						</p>
					</section>
					...
					<section>
						...
						<div className='grid grid-cols-2 gap-2'>
							{(Object.keys(EXPERIENCE_LABELS) as PerformanceExperience[]).map(
								exp => {
									...
									return (
										...
										>
											...
											{isSelected && (
												<div className='w-2 h-2 bg-[#60519B] rounded-full' />
											)}
										</button>
									)
								},
