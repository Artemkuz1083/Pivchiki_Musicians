...
export function Registration() {
	...
	const hasAnyData =
		userName || selectedCities.length > 0 || selectedInstruments.length > 0
			...
			<div className='max-w-md mx-auto p-4 pb-32'>
				...
				<div className='bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6'>
					...
					<div className='space-y-4'>
						{INSTRUMENTS.map(inst => (
							<div
								key={inst}
								className='space-y-3 border-b border-gray-50 last:border-0 pb-4'
							>
								<div className='flex items-center gap-3'>
									...
									<Label
										htmlFor={`inst-${inst}`}
										className='font-bold cursor-pointer flex-1 text-gray-700'
									>
										{inst}
									</Label>
								</div>
								...
								{selectedInstruments.includes(inst) && (
									<div className='ml-7 p-4 bg-[#F8F9FD] rounded-2xl border border-[#60519B]/5'>
										<p className='text-[10px] font-black uppercase text-[#60519B] mb-3 tracking-widest'>
											Уровень владения
										</p>
										<RadioGroup
											value={instrumentLevels[inst]?.toString() || ''}
											onValueChange={val =>
												setInstrumentLevel(inst, Number(val))
											}
										>
				...
				{hasAnyData && (
					<div className='mt-12 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500'>
						<p className='text-center text-[10px] font-black text-gray-400 mb-4 uppercase tracking-[0.3em]'>
							Твоя карточка в поиске
						</p>
