...
export function GroupCard({ group }: GroupCardProps) {
    return (
        <div className='bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-6'>
            ...
            <div className='h-80 bg-gradient-to-br from-[#60519B] to-[#31323E] relative'>
                {group.PhotoURL && (
                    <img
                        src={group.PhotoURL}
                        alt={group.GroupName}
                ...
                <div className='absolute bottom-5 left-6 text-white z-10'> 
                    ...
                    <h2 className='text-3xl font-extrabold'>{group.GroupName}</h2> {/* Больше шрифт и жирность */}
                </div>
            </div>
...
export function DeleteGroupButton({
    ...
    const handleDelete = async () => {
        if (
            !window.confirm(
                `Вы уверены, что хотите полностью удалить группу "${groupName}"? Это действие необратимо.`,
            )
        ) {
            return
        }
